    ...
    if (fetch_result_ok == 0 || fetch_result_ok == MYSQL_DATA_TRUNCATED) {
      ...
      for (i = 0; i < column_count; i++) {
        ...
        if (bind[i].buffer == NULL) {
          char *buffer =
              (char *)calloc(statement->lengths[i] + 1, sizeof(char));
          bind[i].buffer = buffer;
        ...
        if (lua_push == LUA_PUSH_NIL) {
          ...
          if (fields[i].type == MYSQL_TYPE_TIMESTAMP ||
            ...
            snprintf(str, 20, "%d-%02d-%02d %02d:%02d:%02d", t->year, t->month,
            ...
            if (named_columns) {
              LUA_PUSH_ATTRIB_STRING_BY_LENGTH(name, bind[i].buffer,
                                               *bind[i].length);
            } else {
              LUA_PUSH_ARRAY_STRING_BY_LENGTH(d, bind[i].buffer,
                                              *bind[i].length);
            }
          }
...
static int next_iterator(lua_State *L) {
  statement_t *statement = (statement_t *)luaL_checkudata(
      L, lua_upvalueindex(1), DBD_MYSQL_STATEMENT);
...
int dbd_mysql_statement(lua_State *L) {
  ...
  dbd_register(L, DBD_MYSQL_STATEMENT, statement_methods,
