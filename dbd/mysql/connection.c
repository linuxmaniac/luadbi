...
static int connection_new(lua_State *L) {
  ...
  switch (n) {
    case 5:
      if (lua_isnil(L, 5) == 0) port = luaL_checkinteger(L, 5);
    // fallthrough
    case 4:
  ...
  if (!mysql_real_connect(conn->mysql, host, user, password, db, port,
...
int dbd_mysql_connection(lua_State *L) {
  ...
  dbd_register(L, DBD_MYSQL_CONNECTION, connection_methods,
