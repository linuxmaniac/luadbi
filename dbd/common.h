...
#define DBI_ERR_EXECUTE_INVALID \
...
#define DBI_ERR_PARAM_MISCOUNT \
  "Statement expected %d parameters but received %d"
 ...
 */
char *dbd_replace_placeholders(lua_State *L, char native_prefix,
                               const char *sql);

void dbd_register(lua_State *L, const char *name, const luaL_Reg *methods,
                  const luaL_Reg *class_methods, lua_CFunction gc,
