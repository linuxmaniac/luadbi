...
static int statement_affected(lua_State *L) {
  statement_t *statement =
      (statement_t *)luaL_checkudata(L, 1, DBD_SQLITE_STATEMENT);
  ...
  for (p = 2; p <= n; p++) {
    ...
    switch (type) {
      case LUA_TNIL:
        ...
        break;
      case LUA_TNUMBER:
        errflag = sqlite3_bind_double(statement->stmt, i, lua_tonumber(L, p)) !=
                  SQLITE_OK;
        break;
      case LUA_TSTRING: {
        size_t len = -1;
        const char *str = lua_tolstring(L, p, &len);
        errflag = sqlite3_bind_text(statement->stmt, i, str, len,
                                    SQLITE_STATIC) != SQLITE_OK;
        break;
      }
      case LUA_TBOOLEAN:
        errflag = sqlite3_bind_int(statement->stmt, i, lua_toboolean(L, p)) !=
                  SQLITE_OK;
        break;
      default:
        /*
  ...
  if (num_columns) {
    ...
    for (i = 0; i < num_columns; i++) {
      lua_push_type_t lua_push =
          sqlite_to_lua_push(sqlite3_column_type(statement->stmt, i));
      const char *name = sqlite3_column_name(statement->stmt, i);
...
static int next_iterator(lua_State *L) {
  statement_t *statement = (statement_t *)luaL_checkudata(
      L, lua_upvalueindex(1), DBD_SQLITE_STATEMENT);
...
int dbd_sqlite3_statement_create(lua_State *L, connection_t *conn,
  ...
  if (sqlite3_prepare_v2(statement->conn->sqlite, sql_query, strlen(sql_query),
...
int dbd_sqlite3_statement(lua_State *L) {
  static const luaL_Reg statement_methods[] = {
      ...
      {"fetch", statement_fetch},       {"rows", statement_rows},
      {"rowcount", statement_rowcount}, {NULL, NULL}};

  static const luaL_Reg statement_class_methods[] = {{NULL, NULL}};

  dbd_register(L, DBD_SQLITE_STATEMENT, statement_methods,
