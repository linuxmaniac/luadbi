...
static int connection_new(lua_State *L) {
  ...
  if (rc != SQL_SUCCESS) {
    lua_pushnil(L);
    lua_pushfstring(L, DBI_ERR_CONNECTION_FAILED,
                    "Cannot set ODBC version attribute");
    return 2;
  }
  ...
  /* set AUTOCOMMIT off */
  rc = SQLSetConnectAttr(conn->db2, SQL_ATTR_AUTOCOMMIT,
  ...
  rc = SQLConnect(conn->db2, (SQLCHAR *)db, SQL_NTS, (SQLCHAR *)user, SQL_NTS,
...
static int connection_autocommit(lua_State *L) {
  connection_t *conn =
      (connection_t *)luaL_checkudata(L, 1, DBD_DB2_CONNECTION);
  ...
  if (conn->db2) {
    ...
    SQLRETURN rc = SQLSetConnectAttr(conn->db2, SQL_ATTR_AUTOCOMMIT,
                                     (SQLPOINTER)onval, SQL_NTS);
...
int dbd_db2_connection(lua_State *L) {
  ...
  dbd_register(L, DBD_DB2_CONNECTION, connection_methods,
