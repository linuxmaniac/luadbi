...
static int statement_affected(lua_State *L) {
  statement_t *statement =
      (statement_t *)luaL_checkudata(L, 1, DBD_DB2_STATEMENT);
  SQLINTEGER affected;
...
static int statement_execute(lua_State *L) {
  ...
  for (p = 2; p <= n; p++) {
    ...
    switch (type) {
      case LUA_TNIL:
        rc = SQLBindParameter(statement->stmt, i, SQL_PARAM_INPUT, SQL_C_LONG,
                              SQL_INTEGER, 0, 0, (SQLPOINTER)0, 0,
                              (SQLPOINTER)&nullvalue);
      ...
      case LUA_TNUMBER:
        ...
        *num = lua_tonumber(L, p);
        ...
        rc = SQLBindParameter(statement->stmt, i, SQL_PARAM_INPUT, SQL_C_DOUBLE,
                              SQL_DECIMAL, 10, 0, (SQLPOINTER)num, 0, NULL);
        errflag = rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO;
        break;
      case LUA_TSTRING:
        str = lua_tolstring(L, p, &len);
        rc = SQLBindParameter(statement->stmt, i, SQL_PARAM_INPUT, SQL_C_CHAR,
                              SQL_VARCHAR, 0, 0, (SQLPOINTER)str, len, NULL);
        errflag = rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO;
        break;
      case LUA_TBOOLEAN:
        ...
        *boolean = lua_toboolean(L, p);
        offset += sizeof(int);
        rc =
            SQLBindParameter(statement->stmt, i, SQL_PARAM_INPUT, SQL_C_LONG,
                             SQL_INTEGER, 0, 0, (SQLPOINTER)boolean, len, NULL);
        errflag = rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO;
        break;
      default:
        /*
...
static int statement_fetch_impl(lua_State *L, statement_t *statement,
  ...
  if (!statement->cursor_open) goto nodata;
  ...
  for (i = 0; i < statement->num_result_columns; i++) {
    ...
    if (rs->actual_len == SQL_NULL_DATA) lua_type = LUA_PUSH_NIL;

    if (named_columns) lua_pushstring(L, (const char *)name);
...
static int next_iterator(lua_State *L) {
  statement_t *statement =
      (statement_t *)luaL_checkudata(L, lua_upvalueindex(1), DBD_DB2_STATEMENT);
...
int dbd_db2_statement_create(lua_State *L, connection_t *conn,
   ...
   */
  rc = SQLSetStmtAttr(stmt, SQL_ATTR_DEFERRED_PREPARE,
  ...
  if (statement->num_params > 0) {
    statement->parambuf =
        (unsigned char *)malloc(sizeof(double) * statement->num_params);
  }
  ...
  if (statement->num_result_columns > 0) {
    ...
    for (i = 0; i < statement->num_result_columns; i++) {
       ...
       */
      rc = SQLDescribeCol(stmt, (SQLSMALLINT)(i + 1), resultset[i].name,
                          sizeof(resultset[i].name), &resultset[i].name_len,
                          &resultset[i].type, &resultset[i].size,
                          &resultset[i].scale, NULL);
    ...
    if (buflen > 0) buffer = malloc(buflen);
    ...
    for (i = 0; i < statement->num_result_columns; i++) {
      ...
      rc = SQLBindCol(stmt, (SQLSMALLINT)(i + 1), sqlctype, dataptr, datalen,
...
int dbd_db2_statement(lua_State *L) {
  ...
  dbd_register(L, DBD_DB2_STATEMENT, statement_methods, statement_class_methods,
