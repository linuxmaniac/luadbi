...
static void statement_fetch_metadata(lua_State *L, statement_t *statement) {
  ...
  if (statement->metadata) return;

  statement->bind =
      (bindparams_t *)malloc(sizeof(bindparams_t) * statement->num_columns);
  memset(statement->bind, 0, sizeof(bindparams_t) * statement->num_columns);
  bind = statement->bind;

  for (i = 0; i < statement->num_columns; i++) {
    rc = OCIParamGet(statement->stmt, OCI_HTYPE_STMT, statement->conn->err,
                     (dvoid **)&bind[i].param, i + 1);
    if (rc) {
      OCIErrorGet((dvoid *)statement->conn->err, (ub4)1, (text *)NULL, &errcode,
                  errbuf, (ub4)sizeof(errbuf), OCI_HTYPE_ERROR);
      luaL_error(L, "param get %s", errbuf);
    }

    rc = OCIAttrGet(bind[i].param, OCI_DTYPE_PARAM, (dvoid *)&(bind[i].name),
                    (ub4 *)&(bind[i].name_len), OCI_ATTR_NAME,
                    statement->conn->err);
    if (rc) {
      OCIErrorGet((dvoid *)statement->conn->err, (ub4)1, (text *)NULL, &errcode,
                  errbuf, (ub4)sizeof(errbuf), OCI_HTYPE_ERROR);
    ...
    rc = OCIAttrGet(bind[i].param, OCI_DTYPE_PARAM,
                    (dvoid *)&(bind[i].data_type), (ub4 *)0, OCI_ATTR_DATA_TYPE,
                    statement->conn->err);
    if (rc) {
      OCIErrorGet((dvoid *)statement->conn->err, (ub4)1, (text *)NULL, &errcode,
                  errbuf, (ub4)sizeof(errbuf), OCI_HTYPE_ERROR);
    ...
    rc = OCIAttrGet(bind[i].param, OCI_DTYPE_PARAM, (dvoid *)&(bind[i].max_len),
                    0, OCI_ATTR_DATA_SIZE, statement->conn->err);
    if (rc) {
      OCIErrorGet((dvoid *)statement->conn->err, (ub4)1, (text *)NULL, &errcode,
                  errbuf, (ub4)sizeof(errbuf), OCI_HTYPE_ERROR);
    ...
    rc = OCIDefineByPos(statement->stmt, &bind[i].define, statement->conn->err,
                        (ub4)i + 1, bind[i].data, bind[i].max_len, SQLT_STR,
                        (dvoid *)&(bind[i].null), (ub2 *)0, (ub2 *)0,
                        (ub4)OCI_DEFAULT);
    if (rc) {
      OCIErrorGet((dvoid *)statement->conn->err, (ub4)1, (text *)NULL, &errcode,
                  errbuf, (ub4)sizeof(errbuf), OCI_HTYPE_ERROR);
...
static int statement_affected(lua_State *L) {
  statement_t *statement =
      (statement_t *)luaL_checkudata(L, 1, DBD_ORACLE_STATEMENT);
  int affected;
  int rc;
   ...
   */
  rc = OCIAttrGet((dvoid *)statement->stmt, (ub4)OCI_HTYPE_STMT,
                  (dvoid *)&affected, (ub4 *)0, (ub4)OCI_ATTR_ROW_COUNT,
                  statement->conn->err);
...
int statement_close(lua_State *L) {
  ...
  if (statement->stmt) {
    ...
    rc = OCIHandleFree((dvoid *)statement->stmt,
                       OCI_HTYPE_STMT); /* Free handles */
...
int statement_execute(lua_State *L) {
  ...
  for (p = 2; p <= n; p++) {
    ...
    switch (type) {
      case LUA_TNIL:
        errflag = OCIBindByPos(statement->stmt, &bnd, statement->conn->err, i,
                               NULL, 0, SQLT_CHR, (dvoid *)0, (ub2 *)0,
      ...
      case LUA_TBOOLEAN:
        ...
        errflag =
            OCIBindByPos(statement->stmt, &bnd, statement->conn->err, i, value,
                         strlen(value), SQLT_CHR, (dvoid *)0, (ub2 *)0,
   ...
   * statement type
   */
  rc = OCIAttrGet((dvoid *)statement->stmt, (ub4)OCI_HTYPE_STMT, (dvoid *)&type,
                  (ub4 *)0, (ub4)OCI_ATTR_STMT_TYPE, statement->conn->err);
   ...
   */
  rc = OCIStmtExecute(
      statement->conn->svc, statement->stmt, statement->conn->err,
      type == OCI_STMT_SELECT ? 0 : 1, (ub4)0, (CONST OCISnapshot *)NULL,
      (OCISnapshot *)NULL,
   ...
   */
  rc = OCIAttrGet((dvoid *)statement->stmt, (ub4)OCI_HTYPE_STMT,
                  (dvoid *)&num_columns, (ub4 *)0, (ub4)OCI_ATTR_PARAM_COUNT,
                  statement->conn->err);
...
static int statement_fetch_impl(lua_State *L, statement_t *statement,
  ...
  status = OCIStmtFetch(statement->stmt, statement->conn->err, 1,
  ...
  if (statement->num_columns) {
    ...
    for (i = 0; i < statement->num_columns; i++) {
      lua_push_type_t lua_push =
          oracle_to_lua_push(bind[i].data_type, bind[i].null);
...
static int next_iterator(lua_State *L) {
  statement_t *statement = (statement_t *)luaL_checkudata(
      L, lua_upvalueindex(1), DBD_ORACLE_STATEMENT);
...
int dbd_oracle_statement_create(lua_State *L, connection_t *conn,
  ...
  rc = OCIHandleAlloc((dvoid *)conn->oracle, (dvoid **)&stmt, OCI_HTYPE_STMT, 0,
                      (dvoid **)0);
  rc = OCIStmtPrepare(stmt, conn->err, new_sql, strlen(new_sql),
...
int dbd_oracle_statement(lua_State *L) {
  static const luaL_Reg statement_methods[] = {
      {"affected", statement_affected}, {"close", statement_close},
      {"columns", statement_columns},   {"execute", statement_execute},
      {"fetch", statement_fetch},       {"rowcount", statement_rowcount},
      {"rows", statement_rows},         {NULL, NULL}};

  static const luaL_Reg statement_class_methods[] = {{NULL, NULL}};

  dbd_register(L, DBD_ORACLE_STATEMENT, statement_methods,
               statement_class_methods, statement_gc, statement_tostring);
