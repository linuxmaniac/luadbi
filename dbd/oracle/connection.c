...
static int connection_new(lua_State *L) {
  ...
  OCIHandleAlloc((dvoid *)env, (dvoid **)&svc, OCI_HTYPE_SVCCTX, 0,
                 (dvoid **)0);

  /*
   * connect to database server
   */
  rc = OCILogon(env, err, &svc, user, strlen(user), password, strlen(password),
                db, strlen(db));
  if (rc != 0) {
    char errbuf[100];
    int errcode;
...
static int connection_autocommit(lua_State *L) {
  connection_t *conn =
      (connection_t *)luaL_checkudata(L, 1, DBD_ORACLE_CONNECTION);
...
static int connection_close(lua_State *L) {
  ...
  if (conn->oracle) {
    ...
    if (conn->svc) OCIHandleFree((dvoid *)conn->svc, OCI_HTYPE_ENV);
    if (conn->err) OCIHandleFree((dvoid *)conn->err, OCI_HTYPE_ERROR);
...
int dbd_oracle_connection(lua_State *L) {
  ...
  dbd_register(L, DBD_ORACLE_CONNECTION, connection_methods,
