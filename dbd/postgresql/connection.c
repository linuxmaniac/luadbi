...
static int commit(connection_t *conn) { return run(conn, "COMMIT"); }

static int begin(connection_t *conn) { return run(conn, "BEGIN"); }

static int rollback(connection_t *conn) { return run(conn, "ROLLBACK"); }
...
static int connection_new(lua_State *L) {
  ...
  switch (n) {
    ...
    // fallthrough
    case 4:
      if (lua_isnil(L, 4) == 0) host = luaL_checkstring(L, 4);
    // fallthrough
    case 3:
      if (lua_isnil(L, 3) == 0) password = luaL_checkstring(L, 3);
    // fallthrough
    case 2:
      if (lua_isnil(L, 2) == 0) user = luaL_checkstring(L, 2);
    // fallthrough
    case 1:
      /*
...
static int connection_autocommit(lua_State *L) {
  connection_t *conn =
      (connection_t *)luaL_checkudata(L, 1, DBD_POSTGRESQL_CONNECTION);
...
static int connection_close(lua_State *L) {
  ...
  if (conn->postgresql) {
     ...
     */
    if (!conn->autocommit) rollback(conn);
...
int dbd_postgresql_connection(lua_State *L) {
  ...
  static const luaL_Reg connection_class_methods[] = {{"New", connection_new},
                                                      {NULL, NULL}};

  dbd_register(L, DBD_POSTGRESQL_CONNECTION, connection_methods,
               connection_class_methods, connection_gc, connection_tostring);
