...
static int deallocate(statement_t *statement) {
  ...
  if (!statement->conn->postgresql) {
    ...
    if (status != PGRES_COMMAND_OK && status != PGRES_TUPLES_OK) return 1;
  }
...
static int statement_affected(lua_State *L) {
  statement_t *statement =
      (statement_t *)luaL_checkudata(L, 1, DBD_POSTGRESQL_STATEMENT);
...
static int statement_execute(lua_State *L) {
  ...
  result =
      PQexecPrepared(statement->conn->postgresql, statement->name,
                     num_bind_params, (const char **)params, NULL, NULL, 0);
...
cleanup:
  ...
  if (!result) {
    lua_pushboolean(L, 0);
    lua_pushfstring(L, DBI_ERR_ALLOC_RESULT,
                    PQerrorMessage(statement->conn->postgresql));
    return 2;
  }
  ...
  for (i = 0; i < num_columns; i++) {
    ...
    if (PQgetisnull(statement->result, tuple, i)) {
      ...
      lua_push_type_t lua_push =
          postgresql_to_lua_push(PQftype(statement->result, i));
...
static int next_iterator(lua_State *L) {
  statement_t *statement = (statement_t *)luaL_checkudata(
      L, lua_upvalueindex(1), DBD_POSTGRESQL_STATEMENT);
...
int dbd_postgresql_statement_create(lua_State *L, connection_t *conn,
  ...
  if (!result) {
    lua_pushnil(L);
    lua_pushfstring(L, DBI_ERR_ALLOC_STATEMENT,
                    PQerrorMessage(conn->postgresql));
    return 2;
  }
...
int dbd_postgresql_statement(lua_State *L) {
  ...
  dbd_register(L, DBD_POSTGRESQL_STATEMENT, statement_methods,
